...
The Hash Streamer enables consumers to read a stream of bytes that can be cryptographically verified against the requested multihash (target multihash). It relies on an indexing layer, defined in the [Index Specification](/hash-stream/specs), to determine the location of the target data within a given **PackStore** (MAY be spread in multiple location). These bytes can represent a **Blob**, a **Pack**, or a **Containing** cryptographic reference.
...
1. **Index Maintains Metadata**
   ...
   - The [Index](/specs) maintains records mapping Multihashes to the location of their bytes.
...
![hash-streamer-diagram](../../../assets/spec-diagrams/hash-streamer.png)
